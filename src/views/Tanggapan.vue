      ...
      <table class="table shadow">
        <thead class="thead-dark">
          <tr>
            ...
            <th scope="col" class="w-25">Laporan</th>
        ...
        <tbody>
          ...
          /></td>
            ...
            <td>{{ item.status }}</td>
            <td>
              <router-link :to="'/detail/'+item.id" class="btn btn-info">Detail</router-link>
              <button v-on:click="deleteItem(item.id)" class="btn btn-danger">Hapus</button>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.axios
        .get("/pengaduan?page="+this.page, conf)
        .then((response) => {
